  ...
  render() {
    return (
      <header className="bg-white  dark:bg-black p-4 flex justify-center items-center relative">
        <div className="hidden  md:flex justify-center dark:hover:shadow-teal-400 hover:text-teal-400 bg-white dark:bg-gray-800 rounded-full border-gray-400 border dark:shadow-gray-200 dark:shadow-md hover:shadow-teal-400 hover:shadow-md shadow-lg p-1 absolute mr-4">
          ...
          <NavLink to="/uses" onClick={this.closeMenu}>Uses</NavLink>
        </div>
        <button
          ...
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" class="ml-2 {{ IsDarkMode ? 'fill-white' : 'fill-gray-800' }}"
        ...
        {this.state.isMobileMenuOpen && (
         ...
         >
            ...
            <NavLinkMobile to="/projects" onClick={this.closeMenu}>Projects</NavLinkMobile>
            <NavLinkMobile to="/uses" onClick={this.closeMenu}>Uses</NavLinkMobile>
          </div>
        )}
      </header>
    );
...
const NavLink = ({ to, children, onClick }) => {
  return (
    <Link href={to} onClick={onClick} className="p-2 mx-2 rounded  border-b-4 border-transparent hover:border-teal-400 hover:text-teal-400 active:text-gray-400 active:no-underline">
      {children}
    </Link>
  );
};
...
const NavLinkMobile = ({ to, children, onClick }) => {
  return (
    <Link href={to} onClick={onClick} className="block  text-lg py-2 px-3 rounded-lg hover:text-teal-400 hover:border-b-4 hover:border-teal-400 ">
      {children}
    </Link>
  );
};
